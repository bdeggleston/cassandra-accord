...
public class Node implements ConfigurationService.Listener, NodeTimeService
{
    ...
    public <T> AsyncResult<T> withEpoch(long epoch, Supplier<AsyncResult<T>> supplier)
    {
        ...
        {
            configService.fetchTopologyForEpoch(epoch);
            return topology.awaitEpoch(epoch).toChain().flatMap(ignore -> supplier.get().toChain()).beginAsResult();
        }
    }
    ...
    public AsyncChain<Void> forEachLocal(PreLoadContext context, AbstractKeys<?, ?> keys, long minEpoch, long maxEpoch, Consumer<SafeCommandStore> forEach)
    {
        return commandStores.forEach(context, keys, minEpoch, maxEpoch, forEach);
    }

    public AsyncChain<Void> forEachLocalSince(PreLoadContext context, AbstractKeys<?, ?> keys, Timestamp since, Consumer<SafeCommandStore> forEach)
    {
    ...
    public AsyncResult<? extends Outcome> recover(TxnId txnId, Route route)
    {
        {
    ...
    public AsyncResult<? extends Outcome> maybeRecover(TxnId txnId, RoutingKey homeKey, @Nullable AbstractRoute route, ProgressToken prevProgress)
    {
        AsyncResult<? extends Outcome> result = coordinating.get(txnId);
        if (result != null)
            return result;
