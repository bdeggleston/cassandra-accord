...
public class FilteringDepsBuilder
{
    ...
    public static class Builder
    {
        ...
        private boolean isRequired(TxnInfo info)
        {
            ...
            if (!info.status.hasBeen(Status.Committed))
                return true;
            ...
            for (TxnId supportingId : supportingIds)
            {
                ...
                if (!supporting.status.hasBeen(Status.Committed))
                    return true;
        ...
        public <T> void build(Consumer<TxnId> consumer)
        {
            preprocess();
    ...
    public PartialDeps buildPartialDeps(Ranges ranges)
    {
        try (PartialDeps.Builder result = new PartialDeps.Builder(ranges);)
        {
            builders.forEach((seekable, builder) -> builder.build(txnId -> result.add(seekable, txnId)));
            return result.build();
        }
    }
    ...
    {
        try (Deps.Builder result = Deps.builder())
        {
            builders.forEach((seekable, builder) -> builder.build(txnId -> result.add(seekable, txnId)));
            return result.build();
        }
